...
<section class="text-gray-600 dark:text-gray-100 body-font px-4 py-10  ">
    ...
    <div class="flex justify-between align-center mb-sm-2">
         <h2 class="text-gray-700 dark:text-white text-2xl font-bold tracking-tight text-heading md:text-3xl lg:text-4xl"> Licencia </h2>
        <a href="{{ route('licencia.edit', $licencia) }}" class="btn hidden sm:flex align-center bg-sky-500 dark:bg-sky-700 hover:bg-sky-400 dark:hover:bg-sky-600 rounded-full px-3 py-2 shadow-md hover:shadow-xl">
    ...
    <div class="container bg-white rounded shadow-md  px-4 py-4">
        <div class="pb-12">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                Licencia por {{ $licencia->tipo_licencia }} - {{ $licencia->guardavida->nombre ?? '-' }} {{ $licencia->guardavida->apellido ?? '-' }}
            </h3>
            <span class="text-sm text-gray-500 dark:text-gray-300">
                Desde:  {{ $licencia->fecha_inicio->format('d/m/y') }} hasta :  {{ $licencia->fecha_fin->format('d/m/y') }}
            </span>

            <p class="text-sm text-gray-800 dark:text-gray-400 font-medium mt-1 line-clamp-2">
                Playa
                <span class="text-sm text-gray-500 dark:text-gray-300">
                  {{ $licencia->playa->nombre ?? '-' }}
                </span>
            </p>
            <p class="text-sm text-gray-800 dark:text-gray-400 font-medium mt-1 line-clamp-2">
                Puesto
                <span class="text-sm text-gray-500 dark:text-gray-300">
                  {{ $licencia->puesto->nombre ?? '-' }}
                </span>
            </p>
             <p class="text-sm text-gray-800 dark:text-gray-400 font-medium mt-1 line-clamp-2">
                Avisó en tiempo:
                <span class="text-sm text-gray-500 dark:text-gray-300">
                   {!!$licencia->en_tiempo ? 'Si' : 'No' !!}
                </span>
            </p>
             <p class="text-sm text-gray-800 dark:text-gray-400 font-medium mt-1 line-clamp-2">
                Turno
