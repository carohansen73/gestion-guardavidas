...
<div class="text-gray-600 dark:text-gray-100 body-font px-4 ">
    <div class="flex justify-between align-center my-4">
        <h2 class="text-gray-700 dark:text-white text-2xl font-bold tracking-tight text-heading md:text-3xl lg:text-4xl"> Licencias </h2>
        @can('agregar_licencia')
            <a href="{{ route('licencia.create') }}" class="btn hidden sm:flex align-center bg-sky-500 dark:bg-sky-700 hover:bg-sky-400 dark:hover:bg-sky-600 rounded-full px-3 py-2 shadow-md hover:shadow-lg">
...
<div x-data="{ selectedId: null }">
    ...
    {{-- Lista para Mobile --}}
    @include('ui.licencias.partials.index-mobile')

    {{-- Tabla para Desktop --}}
    @include('ui.licencias.partials.index-desktop')
    ...
    <div id="drawer-bottom-example" class="fixed bottom-0 left-0 right-0 z-50 w-full p-4 overflow-y-auto transition-transform translate-y-full bg-white dark:bg-gray-800 " tabindex="-1"
        ...
        <div class="py-4">
            <ul class="space-y-3 font-medium">
                @can('editar_licencia')
                <a :href="'{{ route('licencia.edit', ':id') }}'.replace(':id', selectedId)">
                ...
                <li class="py-2">
                    @can('eliminar_licencia')
                        <form :action="`/licencia/${selectedId}`" method="POST" onsubmit="return confirm('¿Seguro que deseas eliminar esta licencia?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
