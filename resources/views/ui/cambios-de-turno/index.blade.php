...
<div class="text-gray-600 dark:text-gray-100 body-font px-4 ">
    <div class="flex justify-between align-center my-4">
        <h2 class="text-gray-700 dark:text-white text-2xl font-bold tracking-tight text-heading md:text-3xl lg:text-4xl"> Cambios de turno </h2>
        @can('agregar_cambio_turno')
            <a href="{{ route('cambio-de-turno.create') }}" class="btn hidden sm:flex align-center bg-sky-500 dark:bg-sky-700 hover:bg-sky-400 dark:hover:bg-sky-600 rounded-full px-3 py-2 shadow-md hover:shadow-lg">
    ...
    <x-filtros-de-busqueda :playas="$playas" tipo="cambio-de-turno" />
...
<div x-data="{ selectedId: null }">
    ...
    {{-- Lista para Mobile --}}
    @include('ui.cambios-de-turno.partials.index-mobile')

    {{-- Tabla para Desktop --}}
    @include('ui.cambios-de-turno.partials.index-desktop')
    ...
    <div id="drawer-bottom-example" class="fixed bottom-0 left-0 right-0 z-50 w-full p-4 overflow-y-auto transition-transform translate-y-full bg-white dark:bg-gray-800 " tabindex="-1"
        ...
        <h5 id="drawer-bottom-label" class="inline-flex items-center mb-4 text-base font-semibold text-gray-500 dark:text-gray-400">
            ...
            </svg>Opciones de cambio de turno
        </h5>
        ...
        <div class="py-4">
            <ul class="space-y-3 font-medium">
                @can('editar_cambio_turno')
                <a :href="'{{ route('cambio-de-turno.edit', ':id') }}'.replace(':id', selectedId)">
                ...
                @endcan
                <li class="py-2">
                    @can('eliminar_cambio_turno')
                        <form :action="`/cambio-de-turno/${selectedId}`" method="POST" onsubmit="return confirm('¿Seguro que deseas eliminar este cambio de turno?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
