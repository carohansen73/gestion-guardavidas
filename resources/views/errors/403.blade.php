...
<head>
    ...
    <title>Acceso denegado - {{ config('app.name', 'Gestión Guardavidas') }}</title>
...
<body class="font-sans antialiased">
    <div class="relative min-h-screen flex flex-col justify-center items-center overflow-hidden bg-white dark:bg-gray-900 px-6">
        ...
        <div class="relative z-10 w-full sm:max-w-md text-center bg-white dark:bg-gray-800 shadow-md rounded-lg px-8 py-10">
            ...
            <p class="mt-6 text-sm font-semibold tracking-widest text-sky-600 uppercase">Error 403</p>
            ...
            <p class="mt-3 text-sm text-gray-600 dark:text-gray-300">
                {{ $exception->getMessage() && !str_contains($exception->getMessage(), 'This action is unauthorized')
                    ? $exception->getMessage()
                    : 'Tu usuario no cuenta con los permisos necesarios para acceder a esta sección o realizar esta acción.' }}
            </p>

            <div class="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
                <a href="{{ url()->previous() !== url()->current() ? url()->previous() : (auth()->check() ? route('dashboard') : route('welcome')) }}"
