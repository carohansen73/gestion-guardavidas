<div class="space-y-4 sm:hidden">
    <section class="text-gray-600 dark:text-gray-100 body-font px-4 py-4 mb-16">
        <div id="accordion-collapse" data-accordion="collapse" class="bg-white dark:bg-gray-600">
            @foreach ($guardavidasHabilitados as $registro)
                <div class="registro-item-lista rounded " data-playa="{{ $registro->playa->id ?? '' }}">
                    <h2 id="accordion-collapse-heading-{{ $registro->id }}">
                        <button type="button"
                            class="flex items-center justify-between w-full px-4 py-2 font-medium text-left text-gray-500 border border-b-0 border-gray-200 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-800 dark:border-gray-700 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 gap-3"
                            data-accordion-target="#accordion-collapse-body-{{ $registro->id }}" aria-expanded="true"
                    ...
                    <div id="accordion-collapse-body-{{ $registro->id }}" class="hidden"
                        aria-labelledby="accordion-collapse-heading-{{ $registro->id }}">
                                ...
                                <div class="inline-flex rounded-md shadow-xs" role="group">
                                    <a href="{{ route('guardavida.edit', $registro) }}"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20"
                                            stroke-width="1.5" stroke="currentColor" class="w-3 h-3 me-2">
                                    ...
                                    @can('eliminar_guardavida')
                                        <form action="{{ route('user.toggle', $registro->user->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                onclick="return confirm('¿Estás seguro de que querés bloquear a este usuario? Esta acción puede revertirse más tarde.')"
                                    ...
                                    @else
                                        <button disabled
                                            class="inline-flex items-center px-4 py-2 text-sm font-medium bg-gray-100 border border-gray-200 rounded-e-lg hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:text-white dark:hover:bg-gray-700 ">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20"
                                                stroke-width="1.5" stroke="currentColor" class="w-3 h-3 me-2" disabled>
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M18.364 18.364A9 9 0 0 0 5.636 5.636m12.728 12.728A9 9 0 0 1 5.636 5.636m12.728 12.728L5.636 5.636" />
                                            </svg>
                                            Bloquear
    ...
    @can('agregar_guardavida')
        <a href="{{ route('guardavida.create') }}"
            class="btn fixed z-40 flex align-content-center bg-sky-500 dark:bg-sky-700 bottom-24 right-8 rounded-full px-3 py-3 shadow">
